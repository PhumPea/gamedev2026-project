...
@export var bullet_lifetime = 0.25
...
@onready var player_sprite2 : AnimationPlayer = $pzombie/AnimationPlayer
@onready var player_node = $student
@onready var player_node2 = $pzombie
...
func _ready() -> void:
	...
	player_sprite2.animation_finished.connect(_on_animation_finished)
...
func player_animations():
	...
	if is_on_floor():
		if abs(velocity.x) > 0:
			...
			$pzombie/AnimationPlayer.play("walk")
		else:
			#player_sprite.current_animation = "Idle"
			#player_sprite.play("idle")
			$pzombie/AnimationPlayer.play("idle")
	else:
		#player_sprite.current_animation = "Jump"
		#player_sprite.play("jump")
		$pzombie/AnimationPlayer.play("jump")
...
func damage_tween():
	...
	can_damage = false
	for i in range(1,10):
		tween.tween_property(player_node2 , "modulate", Color.RED, 0.1)
		tween.tween_property(player_node2 , "modulate", Color.WHITE, 0.1)
	await tween.finished
	can_damage = true
# --------- SIGNALS ---------- #
...
func shoot():
	...
	$pzombie/AnimationPlayer.play("attack")
	var bullet = bullet_scene.instantiate()
	bullet.global_position = bullet_marker.global_position
	var angle = deg_to_rad(randf_range(0, 20))
	var sign_x = 1.0 if player_node2.scale.x > 0 else -1.0
